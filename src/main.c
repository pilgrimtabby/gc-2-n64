...
// Modified by Pilgrim Tabby (2026) to add new mapping options
...
struct mapping_entry gamecube_to_n64_default_mapping[MAP_GC_NONE + 2] = {
	...
	/* These slots must exist (even pointing at MAP_N64_NONE) for
	 * mapper_change_mapping_entry() to find and overwrite them via
	 * the menu code system. It only updates existing entries, it
	 * never inserts new ones. */
	{ MAP_GC_L_05,  MAP_N64_NONE },
	{ MAP_GC_L_10,  MAP_N64_NONE },
	{ MAP_GC_L_15,  MAP_N64_NONE },
	{ MAP_GC_L_20,  MAP_N64_NONE },
	{ MAP_GC_L_25,  MAP_N64_NONE },
	{ MAP_GC_L_30,  MAP_N64_NONE },
	{ MAP_GC_L_35,  MAP_N64_NONE },
	{ MAP_GC_L_40,  MAP_N64_NONE },
	{ MAP_GC_L_45,  MAP_N64_NONE },
	{ MAP_GC_L_50,  MAP_N64_NONE },
	{ MAP_GC_L_55,  MAP_N64_NONE },
	{ MAP_GC_L_60,  MAP_N64_NONE },
	{ MAP_GC_L_65,  MAP_N64_NONE },
	{ MAP_GC_L_70,  MAP_N64_NONE },
	{ MAP_GC_L_75,  MAP_N64_NONE },
	{ MAP_GC_L_80,  MAP_N64_NONE },
	{ MAP_GC_L_85,  MAP_N64_NONE },
	{ MAP_GC_L_90,  MAP_N64_NONE },
	{ MAP_GC_L_95,  MAP_N64_NONE },
	{ MAP_GC_L_100, MAP_N64_NONE },

	{ MAP_GC_R_05,  MAP_N64_NONE },
	{ MAP_GC_R_10,  MAP_N64_NONE },
	{ MAP_GC_R_15,  MAP_N64_NONE },
	{ MAP_GC_R_20,  MAP_N64_NONE },
	{ MAP_GC_R_25,  MAP_N64_NONE },
	{ MAP_GC_R_30,  MAP_N64_NONE },
	{ MAP_GC_R_35,  MAP_N64_NONE },
	{ MAP_GC_R_40,  MAP_N64_NONE },
	{ MAP_GC_R_45,  MAP_N64_NONE },
	{ MAP_GC_R_50,  MAP_N64_NONE },
	{ MAP_GC_R_55,  MAP_N64_NONE },
	{ MAP_GC_R_60,  MAP_N64_NONE },
	{ MAP_GC_R_65,  MAP_N64_NONE },
	{ MAP_GC_R_70,  MAP_N64_NONE },
	{ MAP_GC_R_75,  MAP_N64_NONE },
	{ MAP_GC_R_80,  MAP_N64_NONE },
	{ MAP_GC_R_85,  MAP_N64_NONE },
	{ MAP_GC_R_90,  MAP_N64_NONE },
	{ MAP_GC_R_95,  MAP_N64_NONE },
	{ MAP_GC_R_100, MAP_N64_NONE },
			...
			switch (g_n64_buf[1]) {
					...
					if (len >= 4) {
						...
						if (id >= 0 && id <= 4) {
							...
							if (chunk == 0) {
                                ...
                                int offset = (chunk - 1) * 32;
                                if (offset >= mapping_size) return 0;
                                
                                int to_copy = mapping_size - offset;
                                if (to_copy > 32) to_copy = 32;
                                
                                memcpy((void*)g_n64_buf, mapping_pointer + offset, to_copy);
                                return to_copy;
                            }
				...
				case 0x03: // Set mapping
                    if (len > 3) {
                        ...
                        unsigned char *mapping_pointer = (unsigned char*)&current_mapping[0];
                        
                        /* In 'Set' commands, chunks are 0-indexed. 
                           Chunk 0 = offset 0, Chunk 1 = offset 32, etc. */
                        int offset = chunk * 32;

                        /* If it is the very first chunk, wipe the old mapping clean */
                        if (offset == 0) {
                            memset(mapping_pointer, 0xff, sizeof(current_mapping));
                        }

                        /* Safely copy the incoming data into the array */
                        if (offset < sizeof(current_mapping)) {
                            int to_copy = len - 4;
                            
                            /* Ensure the incoming payload doesn't exceed array bounds */
                            if (offset + to_copy > sizeof(current_mapping)) {
                                to_copy = sizeof(current_mapping) - offset;
                            }
                            
                            memcpy(mapping_pointer + offset, (void*)(g_n64_buf+3), to_copy);
                        }
...
int main(void)
{
	...
	/* Shadow copy of the last N64 output; sent via RetroSpy every cycle. */
	static unsigned char retrospy_shadow[4] = { 0, 0, 0x80, 0x80 };
	...
	/* Initialise RetroSpy serial output (USART0, 115200 baud). */
	retrospy_init();

	/* Load notch calibration from EEPROM (passthrough if none saved). */
	notch_cal_init();
	...
	while(1)
	{
		...
		if (sync_may_poll()) {
			...
			if (g_gcpad->changed()) {
				...
				{
					volatile unsigned char *dst = n64_use_buf1 ? n64_tx_buf0 : n64_tx_buf1;
					n64_status_to_output(g_n64_status, dst);

					/* Update the RetroSpy shadow copy atomically. */
					retrospy_shadow[0] = dst[0];
					retrospy_shadow[1] = dst[1];
					retrospy_shadow[2] = dst[2];
					retrospy_shadow[3] = dst[3];
				}
			...
			/*
			 * Send the current state to RetroSpy on every poll cycle
			 * (regardless of whether the controller state changed) so
			 * the PC application receives a continuous stream.
			 * At 115200 baud, 33 bytes takes ~2.9 ms, well within the
			 * ~16 ms N64 polling interval.
			 */
			retrospy_send(retrospy_shadow);
