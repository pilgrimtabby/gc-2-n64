...
// Modified by Pilgrim Tabby (2026) to add notch calibration addition and erasure.
...
/* ------------------------------------------------------------------ */
/* Notch calibration wizard                                             */
/* ------------------------------------------------------------------ */

/*
 * Slow blink: n blinks with 250 ms on / 150 ms off, then 450 ms gap.
 * Used to signal which notch the user should push to.
 * With VISUAL_BUZZER the LED flashes; without it, the buzzer beeps.
 */
static void slow_blips(int n)
{
#ifdef VISUAL_BUZZER
    int i;
    for (i = 0; i < n; i++) {
...
/*
 * Run the 9-step notch calibration wizard.
 * Entry: called from rmenu_do() (R → Y in the config menu).
 * Returns 0 on success, -1 on abort.
 *
 * Notch order (index matches notch_cal.h):
 *   0 = centre, 1 = N, 2 = NE, 3 = E, 4 = SE,
 *   5 = S, 6 = SW, 7 = W, 8 = NW
 *
 * LED signals:  (step + 1) slow blinks → user positions stick → press A.
 * Press Start at any point to abort without saving.
 */
static int do_notch_calibration(void)
{
    uint8_t step;

    notch_cal_disable();

    for (step = 0; step < NOTCH_CAL_NUM_POINTS; step++) {
        int ev;
        int8_t sx, sy;

        /* Forced single blink for each calibration step */
        slow_blips(1);

        ev = getEvent();
        if (ev != EV_BTN_A) {
            /* Start or any unexpected button = abort */
            notch_cal_enable();
            return -1;
        }
        ...
        gc_get_raw_axes(gc_report, &sx, &sy);
        notch_cal_set_point(step, sx, sy);
...
int rmenu_do()
{
    ...
    switch(ev)
    {
        ...
        case EV_BTN_X:
            /* Factory reset mapping ONLY */
            eeprom_writeDefaults();
            loadMappingId(0);
            break;

        case EV_BTN_A:
            /* Erase notch calibration without affecting buttons */
            notch_cal_erase();
            break;

        case EV_BTN_Y:
            /* R -> Y : run notch calibration wizard */
            res = do_notch_calibration();
            if (res == -1)
                return -1;
            break;
