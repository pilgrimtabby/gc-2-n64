...
char mapper_getButton(struct mapping_controller_unit *input)
{
	switch (input->type)
	{
		...
		case TYPE_NEG_SLD_TO_BTN:
			/* input->thres is already on the same signed scale as
			   input->value (set via THRES_PCT()/THRES_25_POS, mirroring
			   how TYPE_AXIS_TO_BTN uses its threshold directly). The
			   slider value drops as you squeeze, so trigger the button
			   once it drops to or below the threshold. */
			if (input->value <= (signed char)input->thres)
				return 1;
			break;
	}
	return 0;
